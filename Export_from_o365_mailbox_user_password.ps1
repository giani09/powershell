...
#'xxx' | ConvertTo-SecureString -AsPlainText -Force | ConvertFrom-SecureString | Out-File "xxx"

#---- Variables ----#
$OutputPath = "xxx" # Set Path for Export
...
$ClientSecret = Get-Content "xxx" | ConvertTo-SecureString # Get secure ClientSecret
$UserPassword = Get-Content "xxx" | ConvertTo-SecureString # Get secure password for ServiceAccount
...
$UnsecureUserPassword = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR2)
...
function Authentication {
    # Auth call
    $ReqTokenBody = @{
        Grant_Type    = "Password"; # for Auth with ClientSecret and ServiceUser/Password
        client_id     = $ClientID;
        client_secret = $UnsecurePassword;
        Username      = $Username;
        Password      = $UnsecureUserPassword;
        Scope         = $scope;
    }
...
Start-Transcript -Path "xxx" -Append
$start = Get-Date
